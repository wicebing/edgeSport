...
EDGE SPORT 是可直接部署到 GitHub Pages 的繁體中文運動醫學與運動科學知識站。它每週追蹤核心期刊與國際學會，把新研究放回 `inClass/` 課程知識和過往週報中比較，公開呈現可回查來源、證據層級、原創整理表、原創資料圖與限制。
...
7. 讓已登入的 Codex CLI 讀取完整附件並產生結構化週報。
8. 通過來源、篇數、內容層級、表格與資料結構驗證後，自動累加到 `content/weekly-reports.json`。
9. 執行全站內容驗證；接著即可 commit 與 push。
...
發布會更新 `content/weekly-reports.json` 並立即執行完整驗證。網站的「本週整合」會顯示逐篇摘要、重要發現、原創研究速覽表、可核對數據圖、inClass 對照、既有週報對照、實務意涵、限制與原始連結。
